...
class UserRemoveOwner extends React.Component {
  ...
  render() {
    return (
      <>
        <button className={`${buttonStyles.button} btn-reset-style`} id="remove-owner-btn" onClick={this.toggleModal}>
          <FontAwesomeIcon icon={faMinusCircle} size="lg" />
        </button>
        <HoverTooltip text={'Revoke ownership from an owner of this channel.'} targetId="remove-owner-btn" />
        <Modal isOpen={this.state.modal} toggle={this.toggleModal}>
          <ModalHeader toggle={this.toggleModal}>
            <h3>Remove an owner.</h3>
          </ModalHeader>
