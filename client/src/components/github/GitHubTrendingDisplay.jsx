...
class GitHubTrendingDisplay extends React.Component {
  ...
  fetchRepos(query) {
    ...
    axios
      .get(`https://api.github.com/search/repositories?q=${query}&sort=stars&order=desc&per_page=100`)
      .then((res) => {
        this.setState({
          isLoading: false,
          fetchSucceeded: true,
      ...
      <Card>
        <CardBody>
          ...
          {this.state.isLoading ? (
            ...
            repos.slice(currPage * numResults, numResults * (currPage + 1)).map((eachRepo, i) => (
